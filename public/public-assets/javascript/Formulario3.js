...
form.addEventListener("submit", (event) => {
  ...
  const fapellido = form.elements["fapellido"].value.trim();
  const fcorreo = form.elements["fcorreo"].value.trim();
  const fintegrantes = form.elements["fintegrantes"].value.trim();
  const fedad = form.elements["fedad"].value.trim();
  ...
  if (!fnombre || !fapellido || !fcorreo || !fintegrantes || !fedad || !fdistrito || !fcontraseña || !fconfirm_contraseña) {
